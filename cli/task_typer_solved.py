...
def size_of_path(
    ...
    if is_directory(path):
        if recurse:
            print(f"Calculating size of all files inside {path}")
            files = files_in_directory(path)
            summed_size = 0
            for file in files:
                summed_size += get_size_of_file(file)
            # OR, shorter
            summed_size = sum(get_size_of_file(file) for file in files)
            print(f"Total size of all files inside {path} is {summed_size} bytes.")
        else:
            print(f"ERROR, {path} is a directory")
    else:
        size = get_size_of_file(path)
        print(f"File {path} has {size} bytes.")
